...
<template>
  <div
    id="main-view"
    class="space-y-4 gap-4 px-4 md:px-0 sm:grid sm:grid-cols-2 sm:space-y-0 xl:grid-cols-4"
  >
    <div
    ...
    >
      <div>
        ...
        <ul class="divide-y py-2" v-if="notices.articles">
          <li v-for="idx in 5" v-bind:key="idx">
            <RouterLink
              v-if="notices.articles[idx - 1]"
              :to="{
                name: 'NoticeArticle',
                params: { id: notices.articles[idx - 1].articleid },
              }"
              class="py-2 flex justify-between items-center"
            >
              <span class="text-base">{{
                notices.articles[idx - 1].title
              }}</span>
              <span class="text-sm flex items-center">
                <vue-feather type="user" size=".75rem"></vue-feather>
                <span>{{ notices.articles[idx - 1].views }}</span>
                &nbsp;
                <vue-feather type="thumbs-up" size=".75rem"></vue-feather>
                <span>{{ notices.articles[idx - 1].likes_count }}</span>
                &nbsp;
                <vue-feather type="message-square" size=".75rem"></vue-feather>
                <span>{{ notices.articles[idx - 1].replies_count }}</span>
              </span>
            </RouterLink>
            <span
              v-if="!notices.articles[idx - 1]"
